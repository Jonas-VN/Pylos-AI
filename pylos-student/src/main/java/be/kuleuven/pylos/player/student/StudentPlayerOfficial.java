...
public class StudentPlayerOfficial extends PylosPlayer {
    ...
    public void doMove(PylosGameIF game, PylosBoard board) {
        ...
        if (isFirstMove) {
            ...
            bestMove = minimax(game.getState(), board, firstMove, this.PLAYER_COLOR, MAX_DEPTH, Integer.MIN_VALUE, Integer.MAX_VALUE, true);
        }
    ...
    public void doRemove(PylosGameIF game, PylosBoard board) {
        Move firstMove = new Move();
        Move bestMove = minimax(game.getState(), board, firstMove, this.PLAYER_COLOR, MAX_DEPTH, Integer.MIN_VALUE, Integer.MAX_VALUE, true);
    ...
    private Move minimax(PylosGameState gameState, PylosBoard board, Move latestMove, PylosPlayerColor playerColor, int depth, int alpha, int beta, boolean doReduction) {
        ...
        if (playerColor == this.PLAYER_COLOR) {
            ...
            for (final Move move : latestMove.getChildren()) {
                ...
                // Apply LMR for non-first moves
                if (doReduction && moveCount++ > 0) {
                    depth--;
                }

                Move bestNextMove = minimax(simulator.getState(), board, move, simulator.getColor(), depth - 1, alpha, beta, true);
...
class BoardEvaluator {
    ...
    private int evaluateHigherZLevels(PylosBoard board, PylosPlayerColor playerColor) {
        ...
        // Assign weights based on Z-level
        int ownZWeight = 3; // Adjust as needed
        int otherZWeight = -3; // Adjust as needed

        for (PylosSphere sphere : board.getSpheres(playerColor)) {
            PylosLocation location = sphere.getLocation();
            if (location != null) {
                int z = location.Z;
                if (playerColor == sphere.PLAYER_COLOR) {
                    own_score += z * ownZWeight;
                } else {
                    other_score += z * otherZWeight;
                }
            }
        }
